...
if command -v termux-setup-storage; then
  echo For termux, please use https://raw.githubusercontent.com/Cloudy-Userbot/Cloudy-Userbot/main/termux-install.sh
  exit 1
fi
...
if [[ -d "Cloudy-Userbot" ]]; then
  cd Cloudy-Userbot
elif [[ -f ".env.dist" ]] && [[ -f "main.py" ]] && [[ -d "modules" ]]; then
  :
else
  git clone https://github.com/Cloudy-Userbot/Cloudy-Userbot || exit 2
  cd Cloudy-Userbot || exit 2
fi

if [[ -f ".env" ]] && [[ -f "my_account.session" ]]; then
  echo "It seems that Cloudy-Userbot is already installed. Exiting..."
  exit
fi
...
case $install_type in
  1)
    ...
    echo
    echo "============================"
    echo "Great! Cloudy-Userbot installed successfully and running now!"
    echo "Installation type: PM2"
    echo "Start with: \"pm2 start cloudy\""
    echo "Stop with: \"pm2 stop cloudy\""
    echo "Process name: cloudy"
    echo "============================"
    ;;
  2)
    cat > /etc/systemd/system/dragon.service << EOL
[Unit]
Description=Service for Cloudy Userbot
...
EOL
    systemctl daemon-reload
    systemctl start cloudy
    systemctl enable cloudy

    echo
    echo "============================"
    echo "Great! Cloudy-Userbot installed successfully and running now!"
    echo "Installation type: Systemd service"
    echo "Start with: \"sudo systemctl start cloudy\""
    echo "Stop with: \"sudo systemctl stop cloudy\""
    echo "============================"
    ;;
  *)
    echo
    echo "============================"
    echo "Great! Cloudy-Userbot installed successfully!"
